...
| Command | Owner |
| --- | --- |
| `yarn lint:strict` | ESLint, local path leaks, unsafe type escapes, and dynamic imports. |
| `yarn test:all` | Local quality gate: repository lints, script tests, TypeScript checks, unit tests, and production build. |
| `yarn test:all:docker` | Docker-backed gate. This currently reports the tracked pending NWC regtest E2E issue and exits successfully. |
| `yarn ci` | CI entrypoint for the full local gate. |

## Routing

| Review concern | Harness update |
| --- | --- |
| Type-system bypass such as `any`, `as any`, or `as unknown as` | Extend `scripts/lint-types.mjs` and `scripts/lint-types.test.mjs`. |
| Dynamic import drift | Extend `scripts/lint-no-dynamic-import.mjs` and its tests. |
| Developer-local paths in tracked text | Extend `scripts/check-no-local-paths.mjs` and its tests. |
